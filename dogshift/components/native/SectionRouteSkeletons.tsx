 ...
 * own loading view (real static chrome + grey skeleton list). They render at
 * the group-loading boundary (while the force-dynamic layout resolves), so when
 * the real page mounts — which shows the SAME chrome + the SAME skeleton list
 * during its client fetch — nothing visually changes. The two phases read as
 * ONE continuous skeleton (founder: "j'en veux qu'un pour chaque page, smooth").
 *
 * These mirror the shells' native layout: top padding = safe-area + 2rem,
 * full-width (no card), and a bottom pad that clears the z-50 nav.
 */
...
export function RequestsRouteSkeleton() {
  return (
    <div className="w-full px-3" style={padStyle}>
      ...
      {/* Skeleton list — identical to the page's own loading list */}
