...
import { ClipboardList, CalendarClock, Banknote, Hash, MessageCircle, MessageSquareShare } from "lucide-react";
  ...
  return (
    <div className="rounded-3xl border border-slate-100 bg-white/60 p-4 shadow-[0_8px_30px_-12px_rgba(15,23,42,0.06)] backdrop-blur-xl sm:p-6 transition-all duration-500">
      ...
      <div className="mt-6 space-y-4">
        <section className="group rounded-3xl border border-slate-100 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-md hover:border-slate-200">
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="font-mono text-xs font-medium text-slate-700 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 inline-block">{request.id}</p>
            </div>
            <button
              type="button"
              onClick={async () => {
              ...
              className="inline-flex items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-xs font-bold text-slate-700 transition-all hover:bg-slate-50 hover:text-slate-900 active:scale-95 shadow-sm border border-slate-200"
            >
              {copied ? (
                <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="h-3.5 w-3.5 text-emerald-600" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg> <span className="text-emerald-700">Copié</span></>
              ) : (
                <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="h-3.5 w-3.5" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg> Copier</>
              )}
            </button>
          </div>
        </section>
