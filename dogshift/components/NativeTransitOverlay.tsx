...
const NATIVE_LOADER_ID = "ds-native-loader";
...
/**
 * Global function to dismiss the native loader from anywhere.
 * Can be called even if React is unmounted.
 */
export function dismissNativeTransit() {
  if (typeof window === "undefined") return;
  const el = document.getElementById(NATIVE_LOADER_ID);
  if (el) {
    el.style.opacity = "0";
...
/**
 * Creates an ultra-resilient HTML overlay that lives OUTSIDE the React tree.
 * When mounted, it injects a fixed div into document.body.
 * This overlay survives full page reloads and Next.js layout remounts because
 * it is manually injected and managed via vanilla JS.
 */
export default function NativeTransitOverlay() {
  useEffect(() => {
    ...
    // Check if we should even show it
    if (!isLoginTransit()) return;

    // If it already exists, do nothing (it survived a reload)
    if (document.getElementById(NATIVE_LOADER_ID)) return;

    // Create the overlay manually
    const overlay = document.createElement("div");
    overlay.id = NATIVE_LOADER_ID;
    ...
    // Failsafe auto-dismiss after 15s
    const t = setTimeout(() => {
