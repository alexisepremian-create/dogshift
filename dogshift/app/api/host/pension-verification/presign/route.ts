...
export async function POST(req: NextRequest) {
  try {
    ...
    const profile = await db.sitterProfile.findUnique({
      where: { userId },
      select: { sitterId: true, pensionVerifStatus: true },
    });
    ...
    const key = `pension-verification/${profile.sitterId}/${uuidv4()}.${ext}`;
    ...
    console.error("[api][host][pension-verification][presign]", err);
