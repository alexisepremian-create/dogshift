...
type AppStatus = "PENDING" | "CONTACTED" | "ACCEPTED" | "ACTIVATED" | "REJECTED" | "ARCHIVED";
...
function statusTone(status: AppStatus) {
  ...
  if (status === "ARCHIVED") return "border-slate-300 bg-slate-100 text-slate-500";
...
export default function AdminSitterApplicationsClient({ adminCode }: { adminCode?: string }) {
  ...
  async function load(archived = showArchived) {
    ...
    try {
      const url = archived
        ? "/api/admin/pilot-sitter-applications?archived=1"
        : "/api/admin/pilot-sitter-applications";
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- DEFAULT_CALCOM_URL is a stable env-derived constant
  ...
  async function archiveApplication(id: string, e: React.MouseEvent) {
    e.stopPropagation();
    if (archiveLoadingId) return;
    const label = showArchived ? "restaurer" : "archiver";
    if (!window.confirm(`Voulez-vous ${label} cette candidature ?`)) return;
    setArchiveLoadingId(id);
    try {
      const nextStatus = showArchived ? "REJECTED" : "ARCHIVED";
      const res = await fetch("/api/admin/pilot-sitter-applications/status", {
        method: "POST",
        headers: adminHeaders({ "Content-Type": "application/json" }),
        body: JSON.stringify({ id, status: nextStatus }),
      });
      if (!res.ok) { setError("Impossible de modifier le statut."); return; }
      await load();
    } catch {
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(
                ...
                const active = !showArchived && statusFilter === opt.key;
              ...
              {/* Archived tab — separate from active statuses */}
              <button
                type="button"
                onClick={() => {
              ...
              {filteredItems.length === 0 ? (
                ...
                <div key={a.id} className="group relative">
                  <button
                    ...
                    className={
                      "w-full rounded-2xl border px-4 py-3 pr-10 text-left transition " +
                  ...
                  <button
                    ...
                    disabled={archiveLoadingId === a.id}
                    onClick={(e) => void archiveApplication(a.id, e)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-xl border border-transparent text-slate-300 opacity-100 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-500 disabled:opacity-40 sm:opacity-0 sm:group-hover:opacity-100 sm:text-slate-400"
                  >
                    {showArchived ? (
