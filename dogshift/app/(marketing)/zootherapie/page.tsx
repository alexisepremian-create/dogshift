...
import { ArrowRight, Heart, Brain, Leaf, CheckCircle2 } from "lucide-react";
...
export default function ZootherapiePage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <section className="relative bg-gradient-to-b from-slate-50 to-white pt-20 pb-12 sm:pt-24 sm:pb-16 overflow-hidden">
        {/* Decorative blob */}
        <div
          aria-hidden
      ...
      <section className="bg-white py-14 sm:py-20">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {EDU_BLOCKS.map(({ icon: Icon, color, bg, title, body }) => (
              <div
                key={title}
                className="rounded-2xl border border-slate-100 bg-slate-50 p-6 shadow-sm hover:shadow-md transition-shadow"
              >
                <div className={`inline-flex items-center justify-center h-10 w-10 rounded-xl ${bg} mb-4`}>
                  <Icon className={`h-5 w-5 ${color}`} />
                </div>
                <h3 className="font-semibold text-slate-900 mb-2 text-sm sm:text-base">{title}</h3>
