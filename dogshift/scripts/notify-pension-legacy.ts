/**
 * One-shot script: send pension verification notification emails to legacy sitters.
 * Run: npx tsx scripts/notify-pension-legacy.ts
 */
...
async function main() {
  ...
  console.log(`Found ${sitters.length} sitters to notify`);
  ...
  for (const sitter of sitters) {
    ...
    if (!email) {
      console.warn(`No email for sitter ${sitter.sitterId}, skipping`);
      continue;
    }
    ...
    const { html } = renderEmailLayout({
      ...
      extraHtml: `
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:22px;color:#374151;margin-top:16px;">
          ...
          <p style="margin:0 0 12px 0;">
            <strong>Ce que vous devez faire (dans les 30 jours) :</strong>
          </p>
          ...
          <p style="margin:0;color:#6b7280;font-size:13px;">
            Cette vérification est rapide, gratuite et automatique. En cas de question, écrivez-nous à <a href="mailto:support@dogshift.ch" style="color:#6b7280;">support@dogshift.ch</a>.
          </p>
        </div>
      `,
