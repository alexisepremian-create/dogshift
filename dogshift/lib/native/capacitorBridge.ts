...
async function initNativeBridge() {
  ...
  // ── Hide native splash once React mounted ──────────────────────────────
  // The Capacitor native splash (LaunchScreen.storyboard → purple #7c3aed
  // background + inverted white DogShift logo, generated by
  // `scripts/generate-native-splash.mjs`) covers the entire screen
  // (including the status bar / home indicator safe-area zones) from app
  // launch. It's configured with `launchShowDuration: 30000` + 
  // `launchAutoHide: true` in capacitor.config.ts — the 30 s is a safety
  // net for the case where this bridge init never runs (offline boot,
  // catastrophic JS error). In the normal flow we hide it manually right
  // here, as soon as the WebView's React tree has hydrated.
  //
