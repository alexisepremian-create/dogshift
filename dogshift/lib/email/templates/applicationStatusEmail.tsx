...
function defaultPreviewText(status: ApplicationStatusEmailStatus): string {
  switch (status) {
    case "HIGH":
...
// -----------------------------------------------------------------------------
// Plain-text version (used as `text` fallback by Resend / SMTP)
// -----------------------------------------------------------------------------
...
export function ApplicationStatusEmail(props: {
  ...
  const previewText = (props.previewText || defaultPreviewText(props.status)).trim();
