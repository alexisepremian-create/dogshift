...
- **E2E smoke false positive** (PRs #359 → #368 → #371 → revert): smoke test got 63 chars from `body.innerText` because the homepage Suspense boundary never resolved on the Vercel preview within 15 s (Neon cold start + initial sitter query latency). Two failed fixes (wait for `data-page-loader` attribute, wait for body text growth) and finally reverted `app/(marketing)/loading.tsx` to return `null`. **Lesson**: `loading.tsx` files that the user can reach are tested by smoke — they MUST resolve quickly on preview OR return `null`. The static `<NavigationOverlay />` + MutationObserver handoff cover the footer flash; the Suspense fallback PageLoader was belt-and-suspenders we couldn't afford. See [`docs/bugs/e2e-smoke-body-text-too-short.md`](./docs/bugs/e2e-smoke-body-text-too-short.md) for the full saga.
